...
# logs
PROGNAME=$(basename $0)
echo "${PROGNAME}:" >success_zabbix.log
echo "${PROGNAME}:" >error_zabbix.log
#
#############################################################
#
...
chmod 770 psql_db_zabbix.sql 1>>success_zabbix.log 2>>error_zabbix.log
chown postgres:postgres psql_db_zabbix.sql 1>>success_zabbix.log 2>>error_zabbix.log
sudo -u postgres psql -f psql_db_zabbix.sql 1>>success_zabbix.log 2>>error_zabbix.log
if [ $? == 0 ]
then
...
rm psql_db_zabbix.sql 1>>success_zabbix.log 2>>error_zabbix.log
...
<html>
   <head>
      <title> ZABBIX </title>
      <meta http-equiv = "refresh" content = "0; url = http://www.$DOM/zabbix" />
   </head>
   <body>
   </body>
</html>
...
service zabbix-server start 1>>success_zabbix.log 2>>error_zabbix.log
update-rc.d zabbix-server enable 1>>success_zabbix.log 2>>error_zabbix.log
...
service apache2 restart 1>>success_zabbix.log 2>>error_zabbix.log
# HTTPS
echo -e -n "[EN] Do you want to enable HTTPS? / [ES] ¿Desea habilitar HTTPS? \n[Y/n]: "
read
if [[ $REPLY == "y" ]] || [[ $REPLY == "Y" ]]
then
    apt -qq -y install software-properties-common 1>>success_zabbix.log 2>>error_zabbix.log
    apt -qq update 1>>success_zabbix.log 2>>error_zabbix.log
    add-apt-repository ppa:certbot/certbot -y 1>>success_zabbix.log 2>>error_zabbix.log
    apt -qq install python-certbot-apache -y 1>>success_zabbix.log 2>>error_zabbix.log
...
then
    ...
    then
        ...
        wget -q https://raw.githubusercontent.com/y-u-r/nextcloud-zabbix/master/zbx_export_templates.xml 1>>success_zabbix.log 2>>error_zabbix.log
        chmod 775 zbx_export_templates.xml 1>>success_zabbix.log 2>>error_zabbix.log
    fi
    if [[ ! -z $( echo $DZ | grep "2") ]]
    then
        mkdir -p /tmp/zabbix 1>>success_zabbix.log 2>>error_zabbix.log
        cd /tmp/zabbix 1>>success_zabbix.log 2>>error_zabbix.log
        wget -q https://raw.githubusercontent.com/MrCirca/OpenLDAP-Cluster-Zabbix/master/zabbix_openldap_template.xml 1>>success_zabbix.log 2>>error_zabbix.log
        wget -q https://raw.githubusercontent.com/MrCirca/OpenLDAP-Cluster-Zabbix/master/zabbix_openldap_value_mapping.xml 1>>success_zabbix.log 2>>error_zabbix.log
        chmod 775 zabbix_openldap_template.xml zabbix_openldap_value_mapping.xml 1>>success_zabbix.log 2>>error_zabbix.log
    fi
    if [[ ! -z $( echo $DZ | grep "3") ]]
    then
        mkdir -p /tmp/zabbix 1>>success_zabbix.log 2>>error_zabbix.log
        cd /tmp/zabbix 1>>success_zabbix.log 2>>error_zabbix.log
        wget -q https://raw.githubusercontent.com/Grifagor/zabbix-openvpn/master/openvpn.xml 1>>success_zabbix.log 2>>error_zabbix.log
        chmod 775 openvpn.xml 1>>success_zabbix.log 2>>error_zabbix.log
    fi
    if [ $DZ == "a" ]
    then
        mkdir -p /tmp/zabbix 1>>success_zabbix.log 2>>error_zabbix.log
        ...
        wget -q https://raw.githubusercontent.com/MrCirca/OpenLDAP-Cluster-Zabbix/master/zabbix_openldap_value_mapping.xml 1>>success_zabbix.log 2>>error_zabbix.log
        wget -q https://raw.githubusercontent.com/Grifagor/zabbix-openvpn/master/openvpn.xml 1>>success_zabbix.log 2>>error_zabbix.log
        chmod 775 *.xml 1>>success_zabbix.log 2>>error_zabbix.log
    fi
fi
}
#
...
then
    apt -qq update 1>>success_zabbix.log 2>>error_zabbix.log
    apt -qq install -y apache2 postgresql php php-mbstring php-gd php-xml php-bcmath php-ldap php-pgsql 1>>success_zabbix.log 2>>error_zabbix.log
    wget -q https://repo.zabbix.com/zabbix/4.0/$OSI/pool/main/z/zabbix-release/zabbix-release_4.0-3+${OSV}_all.deb 1>>success_zabbix.log 2>>error_zabbix.log
    dpkg -i zabbix-release_4.0-3+${OSV}_all.deb 1>>success_zabbix.log 2>>error_zabbix.log
    apt update 1>>success_zabbix.log 2>>error_zabbix.log
